...
namespace SeaBattleApi.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        [HttpPost("GetToken")]
        public async Task<ActionResult<string>> Login(AuthData authData)
        {
            var user = await repositoryUser.SearchEntryByConditionAsync(s => s.Login == authData.Login
				&& s.Password == authData.Password);
            if (user.Id == 0)
                return NotFound();

            var claimAuth = new List<Claim> {
                new Claim(ClaimTypes.Name, authData.Login),
